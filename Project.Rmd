...
filter_movements_within_restaurant <- function(movements, buffers, restaurants) {

  joined <- st_join(movements, buffers)
  
  # Spatial joins  
  
  joined$In_Restaurant <- !is.na(joined$id)

  joined$Restaurant_ID <- restaurants$Restaurant_ID[joined$In_Restaurant]
   
  if("Person_ID" %in% names(movements)) {
  ...
  selected <- st_drop_geometry(joined) # Remove geometry
  
  # Regular left_join now  
  full_data <- left_join(selected, restaurants, by="Restaurant_ID")

...
filter_movements_within_restaurant <- function(movements, circles, restaurants) {
   ...
   if ("Person_ID" %in% colnames(movements)) {
     joined$Person_ID <- movements$Person_ID
   } else {
     joined$Person_ID <- NA
   }
   
   # Clean up columns
   result <- joined[, !(names(joined) %in% c("geometry", "id"))]
...
filtered_movements <- movements_within_restaurant %>%
  ...
  group_by(Person_ID, Restaurant_ID) %>%
...
final_report <- agg_data %>%
  ...
  mutate(Person_ID = coalesce(`Restaurant_ID`, Person_ID)) %>%
